function [ outputImage ] = myPatchBasedFiltering( inputImage, W, P, Sigma_I)
%UNTITLED3 Summary of this function goes here
[SizeX, SizeY] = size(inputImage);
outputImage = 255*ones(SizeX,SizeY);
h = waitbar(0, 'Patch Based Filtering in Progress...');
    ...
    for j = 1:SizeY
        ...
        for k = 1:WinSizeX
            for l = 1:WinSizeY
                ...
                Diff = (SelfCroppedPatch-CroppedPatch).*(myGenerateGaussian(min(PatchSizeX,SelfPatchX),min(PatchSizeY,SelfPatchY),(2*P+1)/3));
                Weight_Window(k,l) = sum(sum(Diff.^2))/(min(PatchSizeX,SelfPatchX)*min(PatchSizeY,SelfPatchY));
                
            end
            
        end
      Weight_Window = exp(-(Weight_Window)/(2*(Sigma_I^2)));%(myGenerateGaussian(WinSizeX,WinSizeY,5));
%     Weight_Window = myGenerateGaussian(WinSizeX,WinSizeY,Sigma_X);
      Weight_Window = Weight_Window/sum(sum(Weight_Window));
      outputImage(i,j) = sum(sum(Window.*Weight_Window));
         
    end
    waitbar(double(i*j)/double(SizeX*SizeY));
end
close(h);
end
