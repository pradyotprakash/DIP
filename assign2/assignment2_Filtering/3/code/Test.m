        ...
        for k = 1:WinSizeX
            for l = 1:WinSizeY
                ...
                Diff = (SelfCroppedPatch-CroppedPatch);%.*(myGenerateGaussian(min(PatchSizeX,SelfPatchX),min(PatchSizeY,SelfPatchY),Sigma_I));
                Weight_Window(k,l) = sum(sum(Diff.^2))/(min(PatchSizeX,SelfPatchX)*min(PatchSizeY,SelfPatchY));
                
            end
        end
        Weight_Window = exp(-(Weight_Window)/(2*(Sigma_I^2))).*(myGenerateGaussian(WinSizeX,WinSizeY,Sigma_X));
